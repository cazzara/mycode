...
def bootstrapper(ip_addr, dev_type, config_loc):
    conn = ConnectHandler(device_type=dev_type, ip=ip_addr, username="admin", password="alta3")
    print("Sending config to device, this might take a minute...")
    try:
        conn.enable()
        conn.send_config_from_file(config_loc)
        return True
...
def main():
    ...
    ## Ask user if they want to upload a new config file

    new_conf = input("Do you want to upload a new config file? (y/n)")
    if new_conf[0].lower() == 'y':
        config = input("Where is the config file location?\n")
        ip = input("Enter the IPv4 Address of the device you'd like to configure: \n")
        driver = ""
        try:
            driver = entry[ip]
            if bootstrapper(ip, driver, config):
                print("Config successfully uploaded!")
            else:
                print("There was an error uploading the config...")
        except KeyError:
            print("Sorry that IP address was not in the device list...")
