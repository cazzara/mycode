...
def get_ip_data():
    ...
    input_hostname = input("What is the devices's hostname?")
    input_mac = input("What is the MAC Address?")
    input_driver = input("What is the driver associated with this device? ")
    d = {"IP": input_ip, "hostname": input_hostname, "mac": input_mac, "driver": input_driver}
    return d
...
pyexcel.save_as(records=mylistdict, dest_file_name="{}.xls".format(filename))
